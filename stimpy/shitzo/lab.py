...
class ShitzoLab:
    def __init__(self,repository,config,traders=None):
        self.repository=repository;self.config=config;self.traders=tuple(traders or (TrendTrader(config.shitzo_trend_threshold),MomentumTrader(config.shitzo_momentum_threshold),ContrarianTrader(config.shitzo_contrarian_threshold)))
    ...
    def start(self,run_id=None,now=None):
        ...
        now=(now or datetime.now(UTC)).astimezone(UTC);configuration={"symbols":self.config.shitzo_symbols,"starting_balance":self.config.shitzo_starting_balance_usd,"risk_per_trade":self.config.shitzo_risk_per_trade,"automatic":False,"network_provider":None}
