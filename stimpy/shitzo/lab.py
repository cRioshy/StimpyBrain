...
class ShitzoLab:
    def __init__(self,repository,config,traders=None,regime_classifier=None):
        ...
        self.broker=PaperBroker(repository,rules);self.regime_classifier=regime_classifier or MarketRegimeClassifier();self._run_id=None;self._windows={}
    @property
    ...
    def start(self,run_id=None,now=None):
        ...
        now=(now or datetime.now(UTC)).astimezone(UTC);rr=self.regime_classifier.rules;configuration={"symbols":self.config.shitzo_symbols,"starting_balance":self.config.shitzo_starting_balance_usd,"risk_per_trade":self.config.shitzo_risk_per_trade,"min_confidence":self.config.shitzo_min_confidence,"stop_distance_pct":self.config.shitzo_stop_distance_pct,"take_profit_distance_pct":self.config.shitzo_take_profit_distance_pct,"max_holding_seconds":self.config.shitzo_max_holding_seconds,"thresholds":{"trend":self.config.shitzo_trend_threshold,"momentum":self.config.shitzo_momentum_threshold,"contrarian":self.config.shitzo_contrarian_threshold},"regime_classifier":{"version":rr.version,"trend_spread_threshold":rr.trend_spread_threshold,"low_volatility_threshold":rr.low_volatility_threshold,"high_volatility_threshold":rr.high_volatility_threshold},"automatic":self.config.shitzo_autorun,"network_provider":"coinbase-exchange-public-ticker" if self.config.shitzo_autorun else None}
    ...
    def process_tick(self,tick:MarketTick,received_at=None):
        ...
        snapshot=window.snapshot();label=self.regime_classifier.classify(snapshot,self._run_id);snapshot=replace(snapshot,market_regime=label.combined_regime);self.repository.save_snapshot(self._run_id,snapshot);self.repository.save_regime_label(label);decisions=[];opened=[]
        for trader in self.traders:
