...
def build_app(config=None):
    config=config or StimpyConfig.from_env(); config.validate(); store=ObservationStore(config.database_file,config.data_dir,config.jsonl_rotation_bytes); events=InternalEvents(); client=ReadOnlyHttpClient(config.pandorick_base_url,config.request_timeout_seconds,config.max_retries,config.backoff_seconds); adapter=ObservationAdapter(config,store,client,events,ObservationNormalizer(config.max_payload_bytes,config.max_future_skew_seconds)); memory=MemoryService(store,events); learning=LearningService(memory); workflow=WorkflowService(config.database_file); worker=StimpyWorker(config,adapter,store,memory,workflow); api=ReadOnlyAPI(store,memory,learning,build_graph,worker); server=StimpyApiServer(api,config.api_host,config.api_port); return {"config":config,"store":store,"events":events,"adapter":adapter,"memory":memory,"learning":learning,"workflow":workflow,"worker":worker,"api":api,"server":server}
