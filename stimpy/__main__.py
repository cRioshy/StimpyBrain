...
def main():
    ...
    app["worker"].start(); app["server"].start(); app["shitzo_collector"].start()
    try: stopped.wait()
    finally:
        app["shitzo_collector"].stop(); app["server"].stop(); app["worker"].stop(); app["workflow"].repository.close(); app["shitzo_repository"].close(); app["store"].close()
