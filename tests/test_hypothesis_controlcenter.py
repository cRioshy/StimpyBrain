...
class HypothesisControlcenterTests(unittest.TestCase):
    ...
    def test_controlcenter_assets_are_local_responsive_and_hardened(self):
        ...
        script=self.get("/controlcenter/app.js").read().decode(); self.assertIn("/api/stimpy/hypotheses",script); self.assertIn("/api/stimpy/training-runs",script); self.assertIn("/api/stimpy/shitzo/status",script); self.assertIn("/lifecycle",script); self.assertNotIn('method:\"POST\"',script); self.assertNotIn("innerHTML",script); self.assertNotIn(".style.",script)
