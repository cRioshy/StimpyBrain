...
class HypothesisControlcenterTests(unittest.TestCase):
    ...
    def test_controlcenter_assets_are_local_responsive_and_hardened(self):
        response=self.get("/controlcenter"); html=response.read().decode(); self.assertIn("Hypothesen-Controlcenter",html); self.assertIn('name="viewport"',html); self.assertEqual(response.headers["X-Content-Type-Options"],"nosniff"); self.assertIn("frame-ancestors 'none'",response.headers["Content-Security-Policy"])
        css=self.get("/controlcenter/app.css").read().decode(); self.assertIn("@media(max-width:820px)",css); self.assertIn("auto-fit",css)
        script=self.get("/controlcenter/app.js").read().decode(); self.assertIn("/api/stimpy/hypotheses",script); self.assertIn("/api/stimpy/training-runs",script); self.assertIn("/lifecycle",script); self.assertNotIn('method:\"POST\"',script); self.assertNotIn("innerHTML",script); self.assertNotIn(".style.",script)
