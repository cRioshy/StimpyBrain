...
class HypothesisAnalysisTests(unittest.TestCase):
    ...
    def test_reasoning_and_critic_are_complete_persistent_and_idempotent(self):
        ...
        self.store.close();self.store=ObservationStore(self.path,self.root);self.assertEqual(self.store.get_hypothesis_reasoning(first.reasoning_id),first);self.assertEqual(self.store.schema_version,16)
