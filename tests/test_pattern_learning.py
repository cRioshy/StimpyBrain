...
class PatternLearningTests(unittest.TestCase):
    ...
    def test_grouping_is_stable_idempotent_and_persistent(self):
        ...
        self.store.close(); self.store=ObservationStore(self.path,self.root); self.assertEqual(self.store.get_pattern(first.pattern_id).observed_cases,2); self.assertEqual(self.store.schema_version,16)
