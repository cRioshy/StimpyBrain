...
class HypothesisLifecycleTests(unittest.TestCase):
    ...
    def test_reject_is_atomic_persistent_and_idempotent(self):
        ...
        self.store.close(); self.store=ObservationStore(self.path,self.root); self.assertEqual(self.store.get_hypothesis_lifecycle_event(first.event_id),first); self.assertEqual(self.store.schema_version,9)
