...
class IncubationServiceTests(unittest.TestCase):
    ...
    def test_create_is_persistent_idempotent_and_incubating(self):
        ...
        self.store.close(); self.store=ObservationStore(self.path,self.root); self.service=IncubationService(self.store,clock=lambda:self.now); self.assertEqual(self.service.get(first.incubation_id),first); self.assertEqual(self.store.schema_version,16)
