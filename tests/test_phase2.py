...
class Phase2Tests(unittest.TestCase):
    ...
    def test_config_is_disabled_observe_readonly_local(self):
        c=StimpyConfig(); self.assertFalse(c.pandorick_enabled); self.assertEqual(c.mode,"observe"); self.assertTrue(c.read_only); c.validate()
    ...
    def test_store_jsonl_index_fk_and_rotation(self):
        ...
        self.assertTrue(self.store.append(first)[0]); self.assertTrue(self.store.append(second)[0]); self.assertTrue(self.store.foreign_keys_enabled); self.assertEqual(self.store.schema_version,4); self.assertEqual(self.store.count(),2); self.assertGreaterEqual(len(list((self.root/"observations").glob("*.jsonl"))),2)
