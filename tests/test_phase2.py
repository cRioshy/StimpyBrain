...
class Phase2Tests(unittest.TestCase):
    ...
    def test_config_is_disabled_observe_readonly_local(self):
        c=StimpyConfig(); self.assertFalse(c.pandorick_enabled); self.assertEqual(c.mode,"observe"); self.assertTrue(c.read_only); self.assertEqual(c.incubation_default_seconds,3600); self.assertEqual(c.incubation_max_retries,3); self.assertEqual(c.pattern_min_cases,25); self.assertEqual(c.pattern_supported_min_cases,50); self.assertEqual(c.confidence_max_provisional,.70); self.assertEqual((c.hypothesis_min_investigating_cases,c.hypothesis_min_provisional_cases,c.hypothesis_min_supported_cases),(5,25,100)); self.assertEqual((c.hypothesis_min_supported_ratio,c.hypothesis_max_contradicted_ratio),(.70,.30)); c.validate()
    ...
    def test_store_jsonl_index_fk_and_rotation(self):
        ...
        self.assertTrue(self.store.append(first)[0]); self.assertTrue(self.store.append(second)[0]); self.assertTrue(self.store.foreign_keys_enabled); self.assertEqual(self.store.schema_version,11); self.assertEqual(self.store.count(),2); self.assertGreaterEqual(len(list((self.root/"observations").glob("*.jsonl"))),2)
