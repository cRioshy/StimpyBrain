...
class Phase2Tests(unittest.TestCase):
    ...
    def test_store_jsonl_index_fk_and_rotation(self):
        ...
        self.assertTrue(self.store.append(first)[0]); self.assertTrue(self.store.append(second)[0]); self.assertTrue(self.store.foreign_keys_enabled); self.assertEqual(self.store.schema_version,10); self.assertEqual(self.store.count(),2); self.assertGreaterEqual(len(list((self.root/"observations").glob("*.jsonl"))),2)
