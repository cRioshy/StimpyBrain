...
class HypothesisFoundationTests(unittest.TestCase):
    ...
    def test_create_is_stable_idempotent_bounded_and_persistent(self):
        ...
        self.store.close(); self.store=ObservationStore(self.path,self.root); self.assertEqual(self.store.get_hypothesis(first.hypothesis_id),first); self.assertEqual(self.store.schema_version,9)
