...
The prototype is deliberately not connected to the worker, HTTP polling, broker, Telegram or any order path. Evidence, Reasoning and Critic records are immutable-by-ID SQLite projections linked to observations. Incubation persists both reasoning identities and comparison deltas. Pattern Learning links independent Observation/Evidence cases. The Hypothesis foundation stores bounded research questions, append-only supporting/contradicting/neutral evidence and idempotent evaluation snapshots. These services advance only through explicit calls; there is no background scheduler or automatic strategy change.
