...
Shitzo data flow is `public GET ticker -> explicitly enabled continuous collector -> validated tick -> PriceWindow -> frozen FeatureSnapshot -> deterministic regime classifier + deterministic traders -> virtual PaperBroker -> ShitzoRepository -> GET-only API/Controlcenter`. Regime labels are stored separately in schema v16 and joined to trades through the decision's entry snapshot. Historical backfill is idempotent and never rewrites decisions. `ShitzoLab` remains free of networking and threading; the separate collector owns the loop. There is no Evidence bridge, real broker or HTTP write edge.

The Shitzo symbol allowlist contains BTC-USD, ETH-USD, XRP-USD, SOL-USD, ADA-USD and DOGE-USD. All use the same credential-free Coinbase Exchange GET-only adapter and identical paper-research pipeline; no symbol receives special strategy parameters.
...
The Controlcenter's 15-second GET poll uses stable `data-row-id` keys and in-place DOM reconciliation. Window and nested scroll positions, filters, focus, selected Hypothesis and an open Social detail remain client-side UI state. `LIVE FOLLOW` is stored only in browser local storage, defaults off, and reacts only to a new trade, open position, high-interest Social event or increased error count—not to market ticks or WAIT decisions.
