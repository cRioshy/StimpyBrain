...
- `stimpy/observation_store.py`: rotating JSONL and SQLite schema v7.
...
- `stimpy/api.py`: bounded static and dynamic GET projections, including four Hypothesis paths; all write methods return 405.
...
Phase D.1 stores user/system research questions as Hypotheses, never as facts. Each Hypothesis Evidence record must reference existing Observations and is independent by a stable correlation-derived key. Evaluation separates weighted Evidence Ratio from bounded Confidence, requires configurable case counts, quality and source diversity, and preserves contradicting and neutral cases. It is not yet connected to Reasoning, Self Critic, Incubation, Inspiration or Knowledge Graph promotion.

## Storage and commands

`stimpy_data/{observations,memory,state,database,logs}` is local and Git-ignored. Observations use rotating append-only JSONL plus SQLite metadata offsets. Analysis additionally uses `hypotheses`, `hypothesis_evidence` and `hypothesis_evaluations`. SQLite foreign keys are enabled and Stimpy schema migration is version 7. Tests: `python -m compileall -q stimpy tests`; `python -m unittest discover -s tests -v`. Demo: `python -m stimpy.demo_reasoning_prototype`.
...
Hypothesis thresholds and confidence are also illustrative rather than statistically calibrated. D.1 has no automatic data collection, Inspiration Engine, Hypothesis-specific Reasoning/Critic or incubation lifecycle.
